...
import static digitaslbi.ext.idea.plugin.CommandHelper.runLater;
import static digitaslbi.ext.idea.plugin.VirtualFileProcessor.findAssetsFolder;
...
public class AssetsWatcher {
    ...
    private final JavaClassFileProcessor javaProcessor;
    private final XmlResourceFileProcessor xmlProcessor;
    ...
    public AssetsWatcher(Project project) throws TransformerConfigurationException {
        ...
        this.javaProcessor = new JavaClassFileProcessor(new JavaClassGenerator(GENERATED_PACKAGE_NAME), project);
        this.xmlProcessor = new XmlResourceFileProcessor(new XmlResourceGenerator(), project);
    }
    ...
    private class ProcessEvent implements Runnable {
        ...
        private void processEvent(VFileEvent event) {
            ...
            javaProcessor.delete(fontFamilyName, FileType.JAVA, project);
            xmlProcessor.delete(fontFamilyName, FileType.XML, project);
            ...
            if (event instanceof VFileCopyEvent || event instanceof VFileContentChangeEvent) {
                ...
                try {
                    javaProcessor.generate(assetsFolder);
                    xmlProcessor.generate(assetsFolder);
                } catch (Exception e) {
